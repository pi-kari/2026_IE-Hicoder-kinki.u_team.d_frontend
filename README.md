...
### 進捗は「読んだページ数」ではなく「読み終わったページ」

`progress.progress` はその回に**読み終わったページ番号**。104 ページで中断したら 104。

- 到達位置 (`books_list.total_progress`) は `MAX(progress)`。合計ではない。
- その日に進んだぶんは「日の終わりの到達位置 − 日の始まりの到達位置」
  (`getProgressOnDay`)。その日の行を足してはいけない。
- 読み返して小さい番号を入れても到達位置は下がらない。

**MAX にしている理由は同期。** 合計だと、2 端末が同じ範囲を記録したときや
同じ行が 2 度届いたときに、読んでいないページまで進んでしまう。
最大値なら冪等で、順序にも依存しない。

割合 (`tree_ratio`) は `MIN(100, TRUNC(到達位置 / book_pages * 100))`。
**UI の進捗バーには `tree_ratio` を渡すこと。** `total_progress` はページ番号なので、
`max={100}` の `Progress` にそのまま渡すと 100 ページを超える本が全部満タンに見える。

もとは「その回に読んだページ数」を足し込む設計だった。
`drizzle/0003_progress_to_page_reached.sql` が既存行を累積和に置き換えて意味を揃える
(累積和の最大値 = もとの合計 なので、派生カラムも日別集計も値が変わらない)。

### 同じ本を二重に登録させない

ISBN 付きで登録するとき、同じ ISBN の本が本棚にあれば知らせて登録を止める
(`lib/domain/books.ts` の `getBookByIsbn`)。読み取った時点で判定するので、
登録ボタンを押す前に気づく。行き止まりにせず記録画面への導線を出す。

**`(user_id, isbn)` に unique 索引は張らない。** 2 端末が同じ本をオフラインで
登録すると 2 台目の `book.create` が push で一意制約違反になり、順序を保つ
outbox ではそれ以降の全 op が永久に詰まる (`ix_users_username` と同じ話)。
索引は検索用の非 unique だけ張ってある。

その結果、2 端末で別々に登録した本が合流すると同じ ISBN の本が 2 冊並びうる。
自動では統合しない (進捗をどう混ぜるかが一意に決まらず、勝手に消す方が害が大きい)。
`getBookByIsbn` は先に作られた方を返す。
...
- **競合**: `progress` は追記のみなので原理的に競合しない。行が持つのは
  **そのとき読み終わったページ番号**で、到達位置は `MAX` で導出するので、
  2 端末の記録は自然に「進んだ方」へ揃う。**合計ではない**: 同じ範囲を
  両方が記録したり同じ行が 2 度届いたりしても二重計上されない。
  可変なのは `users.username` と `books_list` の行だけで、
