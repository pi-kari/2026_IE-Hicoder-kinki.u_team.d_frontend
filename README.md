...
### ⚠️ 認証が無い — ユーザー ID が実質の資格情報

**このアプリに認証は存在しない。** 移植元の FastAPI からそうで、`user_id` は
localStorage の値をそのまま使い、サーバは一切検証していない。
`GET /api/users/:uid/books` も `PATCH /api/users/:uid` も、ID さえ知っていれば
誰でも叩ける。

同期を入れたことで、この性質の**影響が集中した**:

- `GET /api/sync/pull?user_id=<uuid>` は **1 リクエストでそのユーザーの全行**
  （本と読書履歴すべて）を返す
- `POST /api/sync/push` は誰の行でも書き込める

`user_id` は UUIDv7 で総当たりはできないが、**端末間の引き継ぎのために
画面に表示してユーザーが入力する値**でもある。つまり「知っている人は全部読める」
という設計を、機能として受け入れている。

そのため、同期のペイロードには**同期に不要な個人情報を載せない**方針を採った。
`user_mail_address` は同期せず pull でも返さない（UI から設定する経路も無い）。
これを載せると pull が「ID さえ分かれば誰でも取れるメールアドレス取得 API」に
なってしまう。

**本番で使うなら認証を足すこと。** 最小でも、登録時にサーバが発行する
不透明な端末シークレットを httpOnly cookie に入れ、push / pull の両方で要求し、
`user_id` を資格情報として扱わないようにする。ただしその場合、
2 台目の引き継ぎは「ID を入力する」だけでは済まなくなるので、
引き継ぎの UX ごと設計し直しになる。
